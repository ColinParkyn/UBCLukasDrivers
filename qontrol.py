...
class Qontroller(object):
	...
	def __init__(self, *args, **kwargs):
		...
		# Populate list of convenience commands accessed via attributes
		self.conv_attributes = ['firmware', 'vfull', 'ifull', 'lifetime']
...
# 					print ('breaking from NLINES ({:} lines received; command was {:})'.format(len(lines), command_id))
	...
	def __getattr__(self, attr):
		# Convenience attribute access for certain parameters
		if (attr in self.conv_attributes):
			return self.issue_command (command_id=attr, ch=None, operator='?', n_lines_requested=1)[0][0]
		return super(type(self), self).__getattr__(attr)
...
class QXOutput(Qontroller):
	...
	def __setattr__(self, attr, val):
		...
		if (hasattr(self, 'initialised') and attr in ['v', 'i', 'vmax', 'imax', 'v_full', 'n_chs']):
